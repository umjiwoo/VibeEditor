...
class TemplateServiceImplTest {
	...
	void 템플릿생성_성공() {
		...
		assertEquals(3, templateRepository.findByUserIdAndActive(user.getId()).size());
	}

	@Test
	void 템플릿이름변경_성공() {
		assertEquals("Spring Boot API 서버 구축", template.getTemplateName());
		UpdateTemplateRequest request = new UpdateTemplateRequest(template.getId(), "Spring Boot REST API");
