			...
			* **`option`**: %s
			...
			1.  **구조**: 다음과 같은 명확한 구조를 갖춰주세요.
				...
				* **본론**:
					...
					* **코드 블록 제한 및 분할 규칙**:
						- 모든 코드 블록 내 문자열은 공백 포함 **최대 1500 바이트 이하, 500자 이내**로 엄격히 제한합니다.
						- 1500 바이트 또는 500자를 초과하는 코드는 절대 한 코드 블록에 담지 마십시오.
						- 긴 코드는 반드시 여러 개의 코드 블록으로 나누어 작성해야 하며, 각 코드 블록에는 반드시 순서 번호를 포함해 주세요.
						- 예시:
						  ```java
						  // Part 1
						  ...
